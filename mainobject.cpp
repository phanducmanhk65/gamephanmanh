...
//#include "function.h"
MainObject::MainObject()//Khởi tạo các giá trị ban đầu
{
    frame_=0;
    x_pos_=0;
    y_pos_=0;
...
bool MainObject::LoadImg(std::string path,SDL_Renderer* screen)//Load ảnh nhân vật
{
...
void MainObject::set_clips()//Tạo hiệu ứng chuyển động cho nhân vật
{
...
void MainObject::Show(SDL_Renderer* des)//show ảnh nhân vật
{
...
void MainObject::HandleInputAction(SDL_Event events,SDL_Renderer* screen)//xử lý sự kiện bàn phím
{
...
void MainObject::HandleBullet(SDL_Renderer* des)//xử lý đạn bắn
{
...
void MainObject::RemoveBullet(const  int& idx)//Xóa đạn
{
    int siz = p_bullet_list_.size();
    if(siz>0&&idx<siz)
    {
...
{
    menu_sound[1]= Mix_LoadWAV("sound//beep_.wav");
    int x1=0;
    int x2=0;
    ...
    if(x1>=0 && x2<MAX_MAP_X && y1>=0&&y2<MAX_MAP_Y)
    {
        if(x_val_ >0)
        {
            ...
            {
               Mix_PlayChannel(-1,menu_sound[1],0);
                map_data.title[x1][y2]=0;
                map_data.title[x2][y2]=0;
                Increasemoney();
            }
